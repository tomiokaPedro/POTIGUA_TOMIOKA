...
CREATE SEQUENCE SQ_NroSessao START WITH 1 NOCACHE ORDER ;
CREATE TRIGGER BI_NroSessao BEFORE
  INSERT ON L06_SESSAO FOR EACH ROW WHEN (NEW.NroSessao IS NULL) BEGIN :NEW.NroSessao := SQ_NroSessao.NEXTVAL;
END;
/
...
CREATE SEQUENCE SQ_NroCand START WITH 1 NOCACHE ORDER ;
CREATE TRIGGER BI_NroCand BEFORE
  INSERT ON L09_CANDIDATO FOR EACH ROW WHEN (NEW.NroCand IS NULL) BEGIN :NEW.NroCand := SQ_NroCand.NEXTVAL;
END;
/

CREATE SEQUENCE SQ_CodCargo START WITH 1 NOCACHE ORDER ;
CREATE TRIGGER BI_CodCargo BEFORE
  INSERT ON L10_CARGO FOR EACH ROW WHEN (NEW.CodCargo IS NULL) BEGIN :NEW.CodCargo := SQ_CodCargo.NEXTVAL;
END;
/
