...
AS
   SELECT
      ...
      MEDICO.EMAIL,
      MEDICO.TELEFONE,
      COUNT(RECEITA.NUMERO) over (partition BY medico.crm) AS N_RECEITAS
       FROM
      MEDICO
    INNER JOIN RECEITA
         ON
...
BEGIN
   DELETE
      ...
      receita_numero IN
      (
         SELECT numero FROM receita WHERE medico_crm = v_crm_medico
      );
   DELETE FROM receita WHERE medico_crm = v_crm_medico;
