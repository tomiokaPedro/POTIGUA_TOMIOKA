...
*/
 ...
 CREATE INDEX IFK_medico_Receita ON Receita
    (Medico_crm
    );

 CREATE
    TABLE Receita_has_Remedio
    (
      ...
      FOREIGN KEY(Receita_numero) REFERENCES Receita(numero),
      FOREIGN KEY(Remedio_codigo) REFERENCES Remedio(codigo)
    );

 CREATE INDEX IFK_Rel_04 ON Receita_has_Remedio
    (Receita_numero
    );

 CREATE INDEX IFK_Rel_05 ON Receita_has_Remedio
    (Remedio_codigo
    );
...
CREATE TRIGGER BI_NUMERO_RECEITA BEFORE
   INSERT
      ON receita FOR EACH ROW WHEN
      (
        NEW.numero IS NULL
      )
      BEGIN :NEW.numero := SQ_NUMERO_RECEITA.nextval;
END;
/
